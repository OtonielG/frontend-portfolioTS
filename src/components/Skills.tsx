...
export default function Skills() {
  return (
    ...
    >
      ...
      <ul className="mt-12 grid w-full grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        {skills.map((skill, index) => (
          <li key={skill.name}>
            <motion.article
              className="flex min-h-36 w-full flex-col items-center justify-center gap-3 rounded-xl border border-gray-700/50 bg-gray-800/50 p-5 text-center backdrop-blur-sm transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(255,165,0,0.15)]"
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
